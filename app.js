...
// Elementos de la interfaz (con comprobación segura)
...
function initQuantumParticles() {
    ...
    for (let i = 0; i < 90; i++) {
        quantumParticles.push({
            x: Math.random() * (canvas ? canvas.width : 1200),
...
// Configuración inicial de cargas (Garantizado que aparezcan en pantalla)
let cargas = [
    { id: 1, x: 300, y: 250, q: 20e-9, radius: 22, isDragging: false },
    { id: 2, x: 600, y: 250, q: -20e-9, radius: 22, isDragging: false }
];

let probe = { x: 450, y: 140, radius: 14, isDragging: false };
let draggedObj = null;
...
function renderCargasUI() {
    ...
    cargas.forEach((c, index) => {
        ...
        div.innerHTML = `
            <h4 style="color:#e2e8f0; margin:4px 0;">Carga ${index + 1} (${c.q > 0 ? '+' : (c.q < 0 ? '-' : '0')})</h4>
            <label style="color:#94a3b8; font-size:13px;">Valor: <span style="color:#f8fafc; font-weight:bold;">${(c.q * 1e9).toFixed(0)}</span> nC</label>
            <input type="range" min="-100" max="100" value="${(c.q * 1e9).toFixed(0)}" step="5" data-index="${index}" style="width:100%;">
        `;
...
if (btnAdd) {
    btnAdd.addEventListener('click', () => {
        if (cargas.length >= 5) return;
        const id = cargas.length + 1;
        const posX = 200 + (cargas.length * 60) % (canvas.width - 200);
        const posY = 180 + (cargas.length * 40) % (canvas.height - 200);
        cargas.push({ id, x: posX, y: posY, q: 30e-9, radius: 22, isDragging: false });
        renderCargasUI();
    });
}
...
// Controles de Arrastre con Ratón
...
// 1. Fondo Cuántico Brillante y Lleno de Vida
function drawQuantumBackground() {
    if (chkQuantum.checked === false) return;

    let bgGlow = ctx.createRadialGradient(
        canvas.width / 2, canvas.height / 2, 50,
        canvas.width / 2, canvas.height / 2, Math.max(canvas.width, canvas.height)
    );
    bgGlow.addColorStop(0, '#1e1035');
    bgGlow.addColorStop(0.6, '#0f172a');
...
// 2. Líneas Equipotenciales Exactas a 90 Grados (Marching Squares)
function drawEquipotentialsMarchingSquares() {
    if (chkEqui.checked === false) return;
    ...
    niveles.forEach(vTarget => {
        ctx.beginPath();
        ctx.strokeStyle = vTarget > 0 ? 'rgba(239, 68, 68, 0.65)' : 'rgba(59, 130, 246, 0.65)';
        ctx.lineWidth = 1.5;
        ctx.setLineDash([4, 4]);
...
// 3. Líneas de Campo Perfectas y Flechas (+ -> -)
function drawFieldLinesWithArrows() {
    if (chkField.checked === false) return;
    ...
    seedCharges.forEach(c => {
        ...
        for (let i = 0; i < numLines; i++) {
            ...
            // Dibujar línea continua azul resplandeciente
            if (points.length > 1) {
                ctx.save();
                ctx.shadowBlur = 6;
...
// 4. Renderizado Esferas 3D con Aura de Luz
function drawEsfera3D(x, y, radius, colorBase, signText) {
    let glowGrad = ctx.createRadialGradient(x, y, radius * 0.3, x, y, radius * 3);
